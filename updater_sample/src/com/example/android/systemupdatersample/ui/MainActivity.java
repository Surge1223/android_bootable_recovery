...
public class MainActivity extends Activity {
    private static final String TAG = "SystemUpdaterSample";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            if (!FileTest(this)) {
                Log.d(TAG,  "Successfully did shit");
            } else {
                Log.e(TAG, "Failed to prepare helper file");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public boolean FileTest(Context context) throws IOException {
        File data = context.getFilesDir();
        File configsRoot = new File(getFilesDir().getAbsolutePath() +  "/configs/");
        ...
        try {
            ...
            if (!configsRoot.exists()) {
                ensureDirectoriesExist(configsRoot, true);
    ...
    private boolean copyJson(Context context, String filename) throws IOException {
        File data = context.getFilesDir();
        File configsRoot = new File(data, "/configs/");
        File output = new File(configsRoot, filename);
        InputStream input = null;
        FileOutputStream outputStream = null;

        if (filename.endsWith(".json")) {
            input =  getResources().openRawResource(R.raw.du_ota);
            Log.d(TAG,  "Successfully moved json file");
        } else {
            input = context.getAssets().open(filename);
            Log.e(TAG,  "failed at everything");
        }
        ...
        try {
            ...
            return data.setWritable(true, false) &&
    ...
    boolean placeJson(Context mContext) throws IOException {
        File data = mContext.getFilesDir();
        File configsRoot = new File(data, "/configs/");
        File fuckingJson = new File(configsRoot + "/du_ota.json");
        if (!fuckingJson.exists()) {
            try {
                copyFile("du_ota.json");
            } catch (Exception e) {
                e.printStackTrace();
                Log.d(TAG, "copyFile  failed to copy : " + fuckingJson.toString());
            }
            Log.e(TAG,  "copyFile moved helper file!");
        }
        if (!fuckingJson.exists()) {
            try {
                copyFileOrDir("du_ota.json");
                Log.e(TAG, "copyFileOrDir moved helper file!");
            } catch (Exception e) {
                e.printStackTrace();
                Log.d(TAG, "copyFileOrDir  failed to copy : " + fuckingJson.toString());
            }
        }
    ...
    public void copyFileOrDir(String path) {
        ...
        try {
            ...
            if (assets.length == 0) {
                ...
                String fullPath =  configsRoot.getAbsolutePath() + "/" + path;
    ...
    private void copyFile(String filename) {
        ...
        try {
            ...
            String newFileName = Environment.getDataDirectory()	+ "/data/" + this.getPackageName() + "/configs/" + filename;
